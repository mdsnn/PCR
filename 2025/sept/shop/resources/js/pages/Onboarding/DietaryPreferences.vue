...
<template>
    <div class="mx-auto max-w-md p-6">
        <h1 class="mb-4 text-center text-2xl font-bold">Your Preferences</h1>
        ...
        <div v-if="step === 1">
            ...
            <div class="flex flex-wrap gap-2">
                <button
                    ...
                    :class="['rounded-full border px-3 py-1', form.allergies.includes(item) ? 'bg-green-600 text-white' : '']"
                >
                    {{ item }}
                </button>
            </div>
            <button @click="step++" class="mt-4 rounded bg-green-600 px-4 py-2 text-white">Next</button>
        </div>
        ...
        <div v-if="step === 2">
            ...
            <div class="flex flex-wrap gap-2">
                <button
                    ...
                    :class="['rounded-full border px-3 py-1', form.dietary_restrictions.includes(item) ? 'bg-green-600 text-white' : '']"
                >
                    {{ item }}
                </button>
            </div>
            <div class="mt-4 flex justify-between">
                <button @click="step--" class="rounded border px-4 py-2">Back</button>
